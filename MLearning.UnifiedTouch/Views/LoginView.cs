...
namespace MLearning.UnifiedTouch.Views
{
	...
	public class LoginView : MvxViewController
	{
		...
		void doBindings()
		{
			...
			set.Bind (doRegButton).To (vm=>vm.CreateAccountCommand);
		...
		void setBackground()
		{
			...
			backImage.Image = UIImage.FromFile (filepath + "fondoa.jpg");
			View.Add (backImage);
		}
		...
		**/
		...
		void initBlurEffect()
		{
			blurMask = new YBlurMask (filepath + "fondoa.jpg", 0, 0, YConstants.DeviceWidht, YConstants.DeviceHeight);
			View.Add (blurMask);	
		}
		...
		{
			animateOffset (regView, 0, 200, 0.25f);
		}
